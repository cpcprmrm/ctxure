...
- The relations $\preceq$ and $<:$ are implemented by `ctxure.multidispatch.is_subtype_invariant` and `ctxure.multidispatch.is_subtype_covariant`.
- The relation $\simeq$ is implemented by `ctxure.multidispatch._is_equal_invariant`. It builds on $=$ by adding `Never` as a LHS wildcard and `Any` as a RHS wildcard, and recursing through type parameters. The $=$ relation uses Python's `==` operator. Most hint objects support an appropriate `==` operator. For example `int | str == str | int` is True and `Literal[1, 2] == Literal[2, 1]` is True. However, `==` does not treat a bare generic as equivalent to the same generic parameterized with `Any` (e.g. `list != list[Any]`). To handle this, `_is_equal_invariant` normalizes both sides through `_get_origin_and_args`, which pads a bare generic's missing type parameters with `Any` (using `get_type_params`) before comparing origins and arguments.
- The function `resolve_base_args` is implemented by `ctxure._typeutil.resolve_base_args`.
- We define `ctxure._typeinfo.TypeInfo` and `ctxure._typeinfo._type_info`, to provide simulated `__orig_bases__`, `__mro__`, and `__parameters__` for built-in generic types. It is to handle these problems.
...
- The literal interest set for top-level `Literal` hints is maintained on `ctxure.multidispatch.Multimethod` as `literal_interest`, a per-position `list` whose entries are either `None` (no registered literals at that position) or a `dict[(type, value), Literal[v]]` with pre-built `Literal[v]` forms. The value-based promotion is gated by method rebinding rather than an explicit position check: while every position's interest set is empty, `Multimethod.arg_type` is the plain `arg_type` method, so the fast path is identical to the pre-Literal implementation. As soon as `_update_literal_interest` records any literal, it rebinds the instance's `arg_type` to `_arg_type_with_literal(pos, arg)`, which does a per-position lookup in `literal_interest` before falling back to the normal runtime-type rules. The interest set is updated by `register()` (via `_update_literal_interest`) and copied by `copy()` (which also re-establishes the `arg_type` rebinding).
