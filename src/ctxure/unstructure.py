...
def unstructure_default(ctx: CtxImpl, data: Any, keymap: dict[str, str] | None = None) -> Any:
    _unstructure_default = config._dispatcher.get().dispatch["_unstructure_default"]
    exe_holder = ctx._site.exe
    if (exe := exe_holder.default_exe) is None:
        exe = _unstructure_default(ctx, data, keymap=keymap)
...
def unstructure_by_type(ctx: CtxImpl, data: Any) -> Any:
    ...
    if type_only_ctx is None or exe is None:
        ...
        exe = _unstructure_exe(type_only_ctx, data)
